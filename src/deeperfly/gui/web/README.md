...
- `static/` — the source, served at `/static/`:
  - `app.js` — controller: layout (grid / focus + thumbnails), frame scrubbing,
    ...
    any of click, double-click, or drag) then set its state via one chip row
    (Detected / Ground truth / Projected) + Reset — undo / redo, the labelled-frames list, the 3D view,
  ...
  - `poseView.js` — one `<canvas>` per camera: frame + draggable skeleton overlay,
    with zoom + pan on the large view(s): mouse wheel or trackpad pinch zooms toward the
    cursor, and either a drag on empty space or a trackpad two-finger scroll pans (a
    wheel event is routed to zoom vs pan by classifying the device — see `onWheel`).
    "Reset view" (the Layout menu, or `0`) returns every camera to the tight fit. Each joint's marker style
