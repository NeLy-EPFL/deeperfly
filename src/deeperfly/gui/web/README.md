...
- `static/` — the source, served at `/static/`:
  - `app.js` — controller: layout (grid / focus + thumbnails), frame scrubbing,
    ...
    skeleton + mesh), hover/selection, the Confirm (point / whole frame) and Discard
    (point in view / all views / whole frame) groups, undo / redo, the labelled-frames
    list, the 3D view, the keypoint-locations reference link (opens the docs viewer in
    a new tab) + keyboard-shortcut help, and edit routing. There is one unified editing
  ...
  - `poseView.js` — one `<canvas>` per camera: frame + draggable skeleton overlay,
    ...
    derived by reprojecting the 3D (a hollow circle in the point's left/right palette
