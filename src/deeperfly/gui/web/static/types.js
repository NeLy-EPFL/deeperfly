 ...
 * @property {string} skeleton_label  `<name>@<digest>`; the name alone is not an identity
 * @property {[number, number][]} edges
 * @property {[number, number, number][]} point_colors  0-255 RGB, one per point
 * @property {[number, number, number][]} edge_colors  0-255 RGB, one per EDGE. Not looked up through an endpoint: an edge the skeleton coloured explicitly is not any of its points' colour.
 * @property {{name: string, color: [number, number, number]}[]} color_legend  one label + 0-255 RGB swatch per distinct point colour, for the legend
